...
const ResourceDeleteNotice = ({
  ...
  const handleDelete = async () => {
    ...
    let result = await fetch(
      `${baseURL}/api/v1/projects/${projectId}/resources/${resource.id}`,
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <div>
      ...
      <div className={style.deleteNoticeContainer}>
        ...
        <p className={style.deleteSubTitle}>
          This will delete <span>{resource.data.length} data</span> inside as
          well
        </p>
...
export default ResourceDeleteNotice;
